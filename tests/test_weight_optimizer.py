...
class TestWeightOptimizer(unittest.TestCase):
    def test_two_peaks_1(self):
        """Calculate optimal weights of teddy -> 0 and bunny -> 1. 
        Equal sample sizes."""
    ...
    def test_two_peaks_2(self):
        """Calculate the MI of teddy -> 0 and bunny -> 1. 
    ...
    def test_two_categories_of_teddies(self):
        """Calculate MI of teddy_black, teddy_brown -> 0 and bunny -> 1. 
        Equal sample sizes for each category"""
...
#        """Find optimal MI for a couple of Gaussians"""
#        def _mu_sigma(i):
#            return (1.5*(i+1), (i+1)/4. + 1)
#
#        subsamples_counts = 10 * np.array([257, 563, 533, 440,
#                                           537, 625, 503, 747])
#
#        data = []
#        for i, cnt in enumerate(subsamples_counts):
#            for v in norm(*_mu_sigma(i)).rvs(cnt):
#                data.append((i, [v]))
#
#        wke = WeightedKraskovEstimator(data)
#        mi, ws = wke.calculate_maximized_mi(k=50)
#        print(mi, ws)
